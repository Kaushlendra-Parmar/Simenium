...
// Available models
const MODELS = [
    { name: 'moving_coil_galvanometer.glb', displayName: 'Moving Coil Galvanometer' }
];
...
function initThreeJS() {
    ...
    renderer.setPixelRatio(window.devicePixelRatio);    // Enhanced color accuracy settings to match Blender    renderer.outputEncoding = THREE.sRGBEncoding;
    renderer.toneMapping = THREE.ACESFilmicToneMapping; // More accurate tone mapping
    renderer.toneMappingExposure = 1.8; // Increased exposure for brighter appearance
    ...
    const ambientLight = new THREE.AmbientLight(0xffffff, 5.0); // Increased ambient light intensity
    scene.add(ambientLight);

    // PRIMARY DIRECTIONAL LIGHTS - Enhanced intensities for better coverage
    const directionalLight1 = new THREE.DirectionalLight(0xffffff, 5.0); // Increased intensity
    directionalLight1.position.set(10, 20, 10);
    scene.add(directionalLight1);

    const directionalLight2 = new THREE.DirectionalLight(0xffffff, 4.0); // Increased intensity
    directionalLight2.position.set(-10, 10, -10);
    scene.add(directionalLight2);

    const directionalLight3 = new THREE.DirectionalLight(0xffffff, 3.5); // Increased intensity
    directionalLight3.position.set(0, 15, -15);
    scene.add(directionalLight3);

    const directionalLight4 = new THREE.DirectionalLight(0xffffff, 3.0); // Increased intensity
    directionalLight4.position.set(-15, 5, 0);
    scene.add(directionalLight4);

    const directionalLight5 = new THREE.DirectionalLight(0xffffff, 2.8); // Increased intensity    directionalLight5.position.set(15, 8, -5);
    scene.add(directionalLight5);

    // ENHANCED PANEL LIGHTS - Increased intensities for better illumination
    const rectLight1 = new THREE.RectAreaLight(0xffffff, 2.0, 8, 8); // Increased intensity
    rectLight1.position.set(0, 12, 0);
    rectLight1.lookAt(0, 0, 0);
    scene.add(rectLight1);

    const rectLight2 = new THREE.RectAreaLight(0xffffff, 1.8, 6, 6); // Increased intensity
    rectLight2.position.set(8, 5, 8);
    rectLight2.lookAt(0, 0, 0);
    scene.add(rectLight2);

    const rectLight3 = new THREE.RectAreaLight(0xffffff, 1.6, 6, 6); // Increased intensity
    rectLight3.position.set(-8, 5, -8);
    rectLight3.lookAt(0, 0, 0);
    scene.add(rectLight3);

    const rectLight4 = new THREE.RectAreaLight(0xffffff, 1.4, 5, 5); // Increased intensity
    rectLight4.position.set(0, 3, 10);
    rectLight4.lookAt(0, 0, 0);
    scene.add(rectLight4);    const rectLight5 = new THREE.RectAreaLight(0xffffff, 1.2, 5, 5); // Increased intensity
    rectLight5.position.set(0, 3, -10);
    rectLight5.lookAt(0, 0, 0);
    scene.add(rectLight5);    // ADDITIONAL PANEL LIGHTS - Enhanced six directions systematic coverage
    // Front panel lights (enhanced intensities)
    const frontPanel1 = new THREE.RectAreaLight(0xffffff, 2.5, 8, 6); // Enhanced intensity
    frontPanel1.position.set(0, 8, 12);
    frontPanel1.lookAt(0, 0, 0);
    scene.add(frontPanel1);

    const frontPanel2 = new THREE.RectAreaLight(0xffffff, 2.0, 6, 4); // Enhanced intensity
    frontPanel2.position.set(0, 2, 10);
    frontPanel2.lookAt(0, 0, 0);
    scene.add(frontPanel2);

    // Back panel lights (enhanced intensities)
    const backPanel1 = new THREE.RectAreaLight(0xffffff, 2.5, 8, 6); // Enhanced intensity
    backPanel1.position.set(0, 8, -12);
    backPanel1.lookAt(0, 0, 0);
    scene.add(backPanel1);

    const backPanel2 = new THREE.RectAreaLight(0xffffff, 2.0, 6, 4); // Enhanced intensity
    backPanel2.position.set(0, 2, -10);
    backPanel2.lookAt(0, 0, 0);
    scene.add(backPanel2);

    // Left panel lights (enhanced intensities)
    const leftPanel1 = new THREE.RectAreaLight(0xffffff, 2.3, 6, 8); // Enhanced intensity
    leftPanel1.position.set(-12, 8, 0);
    leftPanel1.lookAt(0, 0, 0);
    scene.add(leftPanel1);

    const leftPanel2 = new THREE.RectAreaLight(0xffffff, 1.8, 4, 6); // Enhanced intensity
    leftPanel2.position.set(-10, 2, 0);
    leftPanel2.lookAt(0, 0, 0);
    scene.add(leftPanel2);

    // Right panel lights (enhanced intensities)
    const rightPanel1 = new THREE.RectAreaLight(0xffffff, 2.3, 6, 8); // Enhanced intensity
    rightPanel1.position.set(12, 8, 0);
    rightPanel1.lookAt(0, 0, 0);
    scene.add(rightPanel1);

    const rightPanel2 = new THREE.RectAreaLight(0xffffff, 1.8, 4, 6); // Enhanced intensity
    rightPanel2.position.set(10, 2, 0);
    rightPanel2.lookAt(0, 0, 0);
    scene.add(rightPanel2);

    // Top panel lights (enhanced intensities)
    const topPanel1 = new THREE.RectAreaLight(0xffffff, 3.0, 10, 10); // Enhanced intensity
    topPanel1.position.set(0, 15, 0);
    topPanel1.lookAt(0, 0, 0);
    scene.add(topPanel1);

    const topPanel2 = new THREE.RectAreaLight(0xffffff, 2.5, 8, 8); // Enhanced intensity
    topPanel2.position.set(0, 12, 0);
    topPanel2.lookAt(0, 0, 0);
    scene.add(topPanel2);

    // Bottom panel lights (enhanced intensities)
    const bottomPanel1 = new THREE.RectAreaLight(0xffffff, 1.8, 8, 8); // Enhanced intensity
    bottomPanel1.position.set(0, -5, 0);
    bottomPanel1.lookAt(0, 0, 0);
    scene.add(bottomPanel1);    const bottomPanel2 = new THREE.RectAreaLight(0xffffff, 1.5, 6, 6); // Enhanced intensity
    bottomPanel2.position.set(0, -3, 0);
    bottomPanel2.lookAt(0, 0, 0);
    scene.add(bottomPanel2);

    // ADDITIONAL STRATEGIC LIGHTING
    // Enhanced bottom directional light
    const bottomLight = new THREE.DirectionalLight(0xffffff, 2.5); // Enhanced intensity
    bottomLight.position.set(0, -10, 0);
    bottomLight.target.position.set(0, 0, 0);
    scene.add(bottomLight);
    scene.add(bottomLight.target);

    // Corner fill lights for even better coverage
    const cornerLight1 = new THREE.RectAreaLight(0xffffff, 1.2, 4, 4);
    cornerLight1.position.set(-8, 6, 8);
    cornerLight1.lookAt(0, 0, 0);
    scene.add(cornerLight1);

    const cornerLight2 = new THREE.RectAreaLight(0xffffff, 1.2, 4, 4);
    cornerLight2.position.set(8, 6, -8);
    cornerLight2.lookAt(0, 0, 0);
    scene.add(cornerLight2);

    const cornerLight3 = new THREE.RectAreaLight(0xffffff, 1.2, 4, 4);
    cornerLight3.position.set(-8, 6, -8);
    cornerLight3.lookAt(0, 0, 0);
    scene.add(cornerLight3);

    const cornerLight4 = new THREE.RectAreaLight(0xffffff, 1.2, 4, 4);
...
function loadModel(modelName) {
    ...
    loadedModels = [];    loader.load(
        ...
        (gltf) => {
            ...
            let scale = 2; // Model scale increased by 50% (was 1)
